...
// Prevenir navegación en enlaces deshabilitados
...
document.querySelectorAll('.project-section, .cta-section').forEach(section => {
...
const carousels = {
    llave: { current: 0, interval: null },
    mantenimiento: { current: 0, interval: null },
    equipos: { current: 0, interval: null }
};
...
function autoSlideCarousel(carouselName) {
    ...
    const slides = container.querySelectorAll('.carousel-slide');
    if (slides.length <= 1) return; // No hacer autoplay si solo hay 1 imagen
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    ['llave', 'mantenimiento', 'equipos'].forEach(carouselName => {
        const container = document.getElementById(`carousel-${carouselName}`);
        if (container) {
